...
def load_hdf_data_cube(filename):
    ...
    data = np.transpose(
        np.array(np.sqrt(fp.get("/entry/instrument/detector/data"))), (2, 0, 1)
    )
    fp.close()
    return data
...
def overlapConstraint(arr1, arr2, exit_wave, alpha):
    ...
    return ne.evaluate(
        "arr1*(1-(gamma*real(abs(arr2)**2)))+(gamma*exit_wave*conj(arr2))"
    )
...
def create_stxm_estimate(img_pix, prb_pix, position_vector, dx, energy_vector, fwhm):
    ...
    lorentz_array = (1.0 / np.pi) * (
        (0.5 * fwhm_pix) / (radial_array ** 2 + (0.5 * fwhm_pix) ** 2)
    )
    ...
    for i in np.arange(npts):
        ...
        image_array[spy_vector[i] : epy_vector[i], spx_vector[i] : epx_vector[i]] += (
            np.abs((energy_vector[i]) * lorentz_array) + 0.0j
        )  # *np.exp(complex(0,1)*0.0)
