...
def qwp(theta):
    ...
    qwp[0, 0] = EXP(-J * PI / 4.0) * (COS(theta) ** 2 + J * SIN(theta) ** 2)
    qwp[0, 1] = EXP(-J * PI / 4.0) * ((1 - J) * SIN(theta) * COS(theta))
    qwp[1, 0] = EXP(-J * PI / 4.0) * ((1 - J) * SIN(theta) * COS(theta))
    qwp[1, 1] = EXP(-J * PI / 4.0) * (SIN(theta) ** 2 + J * COS(theta) ** 2)
...
def hwp(theta):
    ...
    hwp[0, 0] = EXP(-J * PI / 2.0) * (COS(theta) ** 2 - SIN(theta) ** 2)
    hwp[0, 1] = EXP(-J * PI / 2.0) * 2.0 * COS(theta) * SIN(theta)
    hwp[1, 0] = EXP(-J * PI / 2.0) * 2.0 * COS(theta) * SIN(theta)
    hwp[1, 1] = EXP(-J * PI / 2.0) * (SIN(theta) ** 2 - COS(theta) ** 2)
