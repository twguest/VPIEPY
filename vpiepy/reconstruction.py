...
class config:
    ...
    def __init__(self, scan_path,
                 pmodes = 3, amodes = 3, iterations = 1, nruns = 1,
                 npts = 360, wav = 660e-09, det_pix = 2048, pixdx = 6.5e-06,
                 bit_precision = 32, vpie_config_num = 1):
...
def load_data(config):
    ...
    data = np.zeros((config.npts, config.pmodes, config.amodes, config.padpix, config.padpix))
    ...
    if type(config) == "str":
        ...
        for k in trange(config.pmodes, desc = 'loading data'):
            for l in range(config.amodes):
                ...
                config.subdir = "scan_%02d/grid_00_00/" % np.int(config.vpie_config[k*config.pmodes+l,0])
                
                
                config.theta_p[k] = config.vpie_config[k*config.pmodes][1]
                config.theta_a[l] = config.vpie_config[k*config.pmodes+l][2]
                config.theta_p, config.theta_a = np.radians(config.theta_p), np.radians(config.theta_a)
                
                for j in np.arange(config.npts):
                    
                    config.procdir = "processed_r" + str(config.rebpix) + "_p" + str(config.padpix) + "/npy_files/"
        ...
        spec_data = sp.loadtxt(config.scan_path + ("scan_%02d" % np.int(config.vpie_config[0,0])) + '/grid_00_00/final_smarpod_positions.txt', delimiter=',' )
        ...
        config.fft_denom = 1./config.padpix
...
def calc_probe(config):
    ...
    radpix = sp.hypot(*sp.ogrid[-config.padmid:config.padmid,-config.padmid:config.padmid])
    phase_factor = sp.exp((cplx*np.pi/config.wav) * ( config.z43**(-1.) * (config.dx4 * radpix)**2.) )
    prop_factor = sp.exp((cplx*np.pi/config.wav) * ( ( config.z23**-1 + config.z34**(-1.) ) * (config.dx4 * radpix)**2.) ).astype(config.c_dtype)
    
    probe = np.zeros((config.pmodes, 2, config.padpix, config.padpix)).astype(config.c_dtype)
    
    
    for i in range(config.pmodes):
        probe[i] = jones.prop_through_hwp(beam, config.theta_p[i]/2.)
...
def reconstruction(config, data, probe, update = 'object'):
    ...
    config.rho_xx_max = np.max(np.abs(probe[0,0])**2 + np.abs(probe[1,0])**2 + np.abs(probe[2,0])**2)
    config.rho_yy_max = np.max(np.abs(probe[0,1])**2 + np.abs(probe[1,1])**2 + np.abs(probe[2,1])**2)
    ...
    for ii in np.arange(config.nruns):
        ...
        arr_A = np.zeros((config.pmodes, config.padpix, config.padpix)).astype(config.c_dtype)
        arr_B = np.zeros((config.pmodes, config.padpix, config.padpix)).astype(config.c_dtype)
        ...
        for itr in range(config.iterations):
            ...
            if update == 'object' or 'both':
                ...
                temp_arr2 = (sin(config.theta_a[0])*cplx_diff[k,0]) + (sin(config.theta_a[1])*cplx_diff[k,1]) + (sin(config.theta_a[2])*cplx_diff[k,2])
                arr_B[k,:,:] = optics.upstream_prop(temp_arr2)
            
    
            
                obj_tmp[0,0] = trans_crop[0,0] + (config.ddbetavec[itr]/config.rho_xx_max) * ( (np.conj(probe)[0,0]*arr_A[0]) + (np.conj(probe)[1,0]*arr_A[1]) + (np.conj(probe)[2,0]*arr_A[2]) )
                obj_tmp[0,1] = trans_crop[0,1] + (config.ddbetavec[itr]/config.rho_xx_max) * ( (np.conj(probe)[0,1]*arr_A[0]) + (np.conj(probe)[1,1]*arr_A[1]) + (np.conj(probe)[2,1]*arr_A[2]) )
                obj_tmp[1,0] = trans_crop[1,0] + (config.ddbetavec[itr]/config.rho_xx_max) * ( (np.conj(probe)[0,0]*arr_A[0]) + (np.conj(probe)[1,0]*arr_A[1]) + (np.conj(probe)[2,0]*arr_A[2]) )
                obj_tmp[1,1] = trans_crop[1,1] + (config.ddbetavec[itr]/config.rho_xx_max) * ( (np.conj(probe)[0,1]*arr_A[0]) + (np.conj(probe)[1,1]*arr_A[1]) + (np.conj(probe)[2,1]*arr_A[2]) )
