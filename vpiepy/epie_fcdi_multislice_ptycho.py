...
def overlap_constraint(arr1, arr2, exit_wave, alpha):
    ...
    return arr1 * (1 - (gamma * np.abs(arr2) ** 2)) + (
        gamma * exit_wave * np.conj(arr2)
    )
...
dnstreamAngSpecProp = np.fft.fftshift(
    np.exp(J * (-1.0 * prefactor) * (radpix * dk4) ** 2)
)
upstreamAngSpecProp = np.fft.fftshift(
    np.exp(J * (+1.0 * prefactor) * (radpix * dk4) ** 2)
)
...
psi_det_est = optics.downstream_prop(
    psi_foc_est * np.exp(J * phase_z123), noshift="False"
)
phase_z234 = (PI / wl) * ((1 / z23) + (1 / z34)) * (radpix * dx3) ** 2
psi_sam_est = optics.upstream_prop(
    psi_det_est * np.exp(J * phase_z234), noshift="False"
)

# load probe arrays
probe_cube_fname = (
    f"probe_guess_nprbx{nprbx:03d}_nprby{nprby:03d}_r{rebpix:04d}_p{padpix:04d}.npy"
)
...
if load_probe == 1:
    if os.path.exists(scanpath + gridpath + probe_cube_fname):
        ...
        if os.path.exists(probe_guess_fname):
            ...
            print(
                "neither probe_cube nor probe_guess files found! ... loading psi_det_est file"
            )
            new_probe = np.load(scanpath + gridpath + probe_guess_fname)
            new_probe = optics.upstream_prop(
                probe_det_est * np.exp(complex(0, 1) * phase_z234)
            )
        else:
...
if load_object_estimate == 1:
    if os.path.exists(scanpath + gridpath + object_guess_fname0):
        ...
        print(
            "object multi-probe estimate file not found! ... loading object single probe estimate"
        )
        ...
        rand_amp = 0.9 + np.random.rand(fovpix, fovpix) / 10.0
        rand_phase = (np.pi / 2.0) * np.random.rand(fovpix, fovpix) - np.pi / 4
...
for i in np.arange(iterations):
    ...
    for j in np.arange(ptych_num):
        ...
        if i > update_probe_after:
            new_probe[nslices - 1] = overlap_constraint(
                probe_guess[nslices - 1], obj1[nslices - 1], new_esw, alpha_probe
            )

        if i > update_object_after:
            # new_obj1[nslices-1] = overlap_constraint(obj1[nslices-1], probe_guess[nslices-1], new_esw, alpha_object)
            object_guess[nslices - 1, spy:epy, spx:epx] = overlap_constraint(
                obj1[nslices - 1], probe_guess[nslices - 1], new_esw, alpha_object
            )
        ...
        for l in reversed(range(nslices - 1)):
            ...
            if i > update_probe_after:
                new_probe[l] = overlap_constraint(
                    probe_guess[l], obj1[l], new_esw, alpha_probe
                )

            if i > update_object_after:
                object_guess[l, spy:epy, spx:epx] = overlap_constraint(
                    obj1[l], probe_guess[l], new_esw, alpha_object
                )
